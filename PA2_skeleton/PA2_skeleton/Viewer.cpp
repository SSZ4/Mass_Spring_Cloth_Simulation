...
void Viewer::Motion(int x, int y)
{
	...
	if (m_Mouse_Event[0])
	{
		...
		else if (interationMode)
		{
			...
			vec3 force;
			force.x = (mat[0] * diffx + mat[4] * diffy + 0 + mat[12]);
			force.y = -(mat[1] * diffx + mat[5] * diffy + 0 + mat[13]);
			force.z = (mat[2] * diffx + mat[6] * diffy + 0 + mat[14]);
			S_Simulator.cloth->add_force(force);
		}
 	}
	else if (m_Mouse_Event[1])
	{
		m_Translate[0] += (float) 0.3 * diffx;
		m_Translate[1] -= (float) 0.3 * diffy;
	}
...
void Viewer::Keyboard(unsigned char key, int x, int y)
{
	...
	case '3':
		...
		break;
	case'4':
		S_Simulator.is_sphere = !S_Simulator.is_sphere;
		break;
	case 27:
	case 'q':
	case 'Q':
